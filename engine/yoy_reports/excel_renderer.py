...
def _render_sheet(ws, yoy_df_curr, yoy_df_prev, yoy_start_dt, yoy_end_dt, yoy_start_prev, yoy_config, yoy_grouping_col):
    ...
    for group_name, branches in groups.items():
        ...
        idx = 2
        for b in branches:
            apply_style(ws.cell(row=current_row, column=idx, value=yoy_start_prev.year), is_header=True)
...
def render_yoy_sales_excel(yoy_output_path, yoy_df_curr, yoy_df_prev, yoy_start_dt, yoy_end_dt, yoy_start_prev, yoy_config, yoy_grouping_col, yoy_segmented):
    ...
    if yoy_segmented and not df_curr_clean.empty:
        ...
        for p in periods:
            ...
            _render_sheet(ws, df_c, df_p, s_dt, e_dt, s_prev, yoy_config, yoy_grouping_col)
    ...
    if span_days > 366 and not df_curr_clean.empty:
        ...
        for y in years:
            ...
            _render_sheet(ws_full, df_c, df_p, s_dt, e_dt, s_prev, yoy_config, yoy_grouping_col)
    else:
        title = "Full Report" if yoy_segmented else "Sales"
        ws_full = wb.create_sheet(title=title)
        _render_sheet(ws_full, yoy_df_curr, yoy_df_prev, yoy_start_dt, yoy_end_dt, yoy_start_prev, yoy_config, yoy_grouping_col)

    if not wb.sheetnames:
        ws_empty = wb.create_sheet(title="Sales")
        _render_sheet(ws_empty, yoy_df_curr, yoy_df_prev, yoy_start_dt, yoy_end_dt, yoy_start_prev, yoy_config, yoy_grouping_col)
